...
try {
    ...
    $apiUrlSendEvents = @{
        ...
        "ErrorLog" = "https://alex666.pythonanywhere.com/api_guardarCurrent"
    }
    $apiUrlSendMachineInfo = "https://alex666.pythonanywhere.com/api_datosMaquina"

    $eventTypes = @("Application", "Setup", "Security", "System", "ErrorLog")
    ...
    foreach ($eventType in $eventTypes) {
        ...
        if ($eventType -ne "ErrorLog") {
            ...
            if (-not (Get-Module -ListAvailable -Name SqlServer)) {
                Write-Host "SQL Server no instalado."
                Log-EventStatus -machineId $id_maquina -eventType $eventType -eventCount 0 -status "Error" -errorMessage "SQL Server no instalado."
                continue
            }
...
"@
                ...
                foreach ($event in $events) {
                    ...
                    # También registrar el evento en el log local
                    Log-EventStatus -machineId $id_maquina -eventType $eventType -eventCount 1 -status "Info" -errorMessage "$($event.LogDate): $($event.Text)"
                }
